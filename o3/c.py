...
class ConvolutionalNeuralNetworkModel:
    def __init__(self):
        ...
        conv1 = tf.layers.conv2d(self.x, filters=32, kernel_size=[5, 5], strides=[1, 1], padding='same', activation=tf.nn.relu)
        ...
        densed = tf.layers.dense(inputs=flattened, units=1024)

        dropout = tf.layers.dropout(inputs=densed, rate=0.4)

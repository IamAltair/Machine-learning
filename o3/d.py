...
class ConvolutionalNeuralNetworkModel:
    def __init__(self):
        ...
        conv1 = tf.layers.conv2d(self.x, filters=64, kernel_size=[5, 5], strides=[1, 1], padding='same', activation=tf.nn.relu)
        pool1 = tf.layers.max_pooling2d(conv1, pool_size=[2, 2], strides=[2, 2], padding='same')

        conv2 = tf.layers.conv2d(pool1, filters=64, kernel_size=[5, 5], strides=[1, 1], padding='same')
        pool2 = tf.layers.max_pooling2d(conv2, pool_size=[2, 2], strides=[2, 2], padding='same')

        conv3 = tf.layers.conv2d(pool2, filters=32, kernel_size=[5, 5], strides=[1, 1], padding='same')
        pool3 = tf.layers.max_pooling2d(conv3, pool_size=[2, 2], strides=[2, 2], padding='same')

        flattened1 = tf.layers.flatten(pool3)
        densed1 = tf.layers.dense(inputs=flattened1, units=1024)
        dropout = tf.layers.dropout(inputs=densed1, rate=0.4)
